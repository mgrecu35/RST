...
&   temp5)*sumjd*(1-eps)/cos1)*tbb
  ...
  DO i=n+1,1,-1
    temp1 = -(intdo(i)/cos1)
    temp2 = jd(i)/cos1
    tempb4 = dz*EXP(temp1)*sumjdb
    temp3 = -(intup(i)/cos1)
    temp4 = jup(i)/cos1
    tempb3 = dz*EXP(temp3)*sumjub
    jupb(i) = jupb(i) + k(i)*tempb3/cos1
    kb(i) = kb(i) + temp2*tempb4 + temp4*tempb3
    intupb(i) = intupb(i) - EXP(temp3)*temp4*k(i)*dz*sumjub/cos1
    jdb(i) = jdb(i) + k(i)*tempb4/cos1
    intdob(i) = intdob(i) - EXP(temp1)*temp2*k(i)*dz*sumjdb/cos1
  END DO
  DO k1=n+1,2,-1
    tempb2 = dz*0.5*intdob(k1)
    intdob(k1-1) = intdob(k1-1) + intdob(k1)
    kb(k1) = kb(k1) + tempb2
    kb(k1-1) = kb(k1-1) + tempb2
    intdob(k1) = 0.0
  END DO
  DO k1=n,1,-1
    tempb1 = dz*0.5*intupb(n+1-k1)
    intupb(n-k1+2) = intupb(n-k1+2) + intupb(n+1-k1)
    kb(n+1-k1) = kb(n+1-k1) + tempb1
    kb(n+1-k1+1) = kb(n+1-k1+1) + tempb1
    intupb(n+1-k1) = 0.0
  END DO
  ...
  ab = 0.0
  DO k1=n+1,1,-1
    tempb0 = a(k1)*cos1*0.5*jupb(k1)
    temp = i01p(2*k1-1) + i01p(2*k1+1)
    temp0 = i01p(2*k1-1) + i01p(2*k1+1)
    tempb = -(a(k1)*cos1*0.5*jdb(k1))
    ab(k1) = ab(k1) + (cos1*0.5*(temp*g(k1))+i01p(2*k1)-t(k1))*jupb(k1) &
&     + (i01p(2*k1)-cos1*0.5*(temp0*g(k1))-t(k1))*jdb(k1)
    i01pb(2*k1) = i01pb(2*k1) + a(k1)*jdb(k1)
    i01pb(2*k1-1) = i01pb(2*k1-1) + g(k1)*tempb
    i01pb(2*k1+1) = i01pb(2*k1+1) + g(k1)*tempb
    gb(k1) = gb(k1) + temp*tempb0 + temp0*tempb
    jdb(k1) = 0.0
    i01pb(2*k1-1) = i01pb(2*k1-1) + g(k1)*tempb0
    i01pb(2*k1+1) = i01pb(2*k1+1) + g(k1)*tempb0
